import { useState } from 'react';
import { useNavigate, useLocation } from 'react-router-dom';
import { Target, Eye, EyeOff, ArrowLeft } from 'lucide-react';
import { useGoogleLogin } from '@react-oauth/google';
import { toast } from 'react-hot-toast';
import './Auth.css';

const Auth = ({ onLogin }) => {
  const navigate = useNavigate();
  const location = useLocation();
  const [authMode, setAuthMode] = useState(location.state?.mode || 'login'); // 'login' | 'register'
  const [showPassword, setShowPassword] = useState(false);
  
  const getDb = () => {
    let db = localStorage.getItem('ielts_users_db');
    if (!db) {
      db = [
        { id: 1, name: 'Alex (Siz)', email: 'alex@example.com', password: '123', plan: 'free', tgStatus: 'Aktiv Seans' },
        { id: 2, name: 'Sardor', email: 'sardor@example.com', password: '123', plan: 'pro_plus', tgStatus: 'Owner' }
      ];
      localStorage.setItem('ielts_users_db', JSON.stringify(db));
      return db;
    }
    return JSON.parse(db);
  };

  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        // Google'dan real user ma'lumotlarini (email, name) olish
        const userInfoRes = await fetch('https://www.googleapis.com/oauth2/v3/userinfo', {
          headers: { Authorization: `Bearer ${tokenResponse.access_token}` }
        });
        const userInfo = await userInfoRes.json();

        const db = getDb();
        const googleEmail = userInfo.email;
        const googleName = userInfo.name || userInfo.given_name || 'Google User';

        let user = db.find(u => u.email.toLowerCase() === googleEmail.toLowerCase());
        if (!user) {
          // Yangi Google foydalanuvchisini bazaga qo'shish
          user = {
            id: Date.now(),
            name: googleName,
            email: googleEmail,
            password: 'google_oauth',
            plan: 'free',
            tgStatus: 'Google orqali'
          };
          db.push(user);
          localStorage.setItem('ielts_users_db', JSON.stringify(db));
        }

        localStorage.setItem('ielts_current_user_email', user.email);
        localStorage.setItem('ielts_current_user_name', user.name);
        localStorage.setItem('ielts_user_membership_precise', user.plan);
        onLogin();
        toast.success("Muvaffaqiyatli kirdingiz! 🎉");
        navigate('/dashboard');
      } catch (err) {
        console.error('Google userinfo fetch error:', err);
        toast.error("Google ma'lumotlarini olishda xatolik yuz berdi!");
      }
    },
    onError: () => {
      toast.error("Google orqali kirishda xatolik yuz berdi!");
    }
  });

  // Form states
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [name, setName] = useState('');

  const handleAuthSubmit = (e) => {
    e.preventDefault();
    if (!email || !password || (authMode === 'register' && !name)) {
      toast.error("Iltimos, barcha maydonlarni to'ldiring!");
      return;
    }
    
    const db = getDb();

    if (authMode === 'register') {
      const existing = db.find(u => u.email.toLowerCase() === email.toLowerCase());
      if (existing) {
        toast.error("Ushbu email bilan avval ro'yxatdan o'tilgan!");
        return;
      }
      const newUser = {
        id: Date.now(),
        name,
        email: email.toLowerCase(),
        password,
        plan: 'free',
        tgStatus: 'Yangi'
      };
      db.push(newUser);
      localStorage.setItem('ielts_users_db', JSON.stringify(db));
      
      localStorage.setItem('ielts_current_user_email', newUser.email);
      localStorage.setItem('ielts_current_user_name', newUser.name);
      localStorage.setItem('ielts_user_membership_precise', 'free');
      onLogin();
      toast.success("Hisobingiz muvaffaqiyatli yaratildi! 🎉");
      navigate('/dashboard');
    } else {
      // Admin maxsus tekshiruvi
      if (email === 'sardoradmin@gmail.com' && password === 'sardoradmin123') {
        sessionStorage.setItem('ielts_admin_logged', 'true');
        localStorage.setItem('ielts_current_user_name', 'Admin Sardor');
        localStorage.setItem('ielts_user_membership_precise', 'pro_plus');
        onLogin();
        toast.success("Admin CRM profiliga muvaffaqiyatli kirdingiz! 🛡️");
        navigate('/admin');
        return;
      }

      const user = db.find(u => u.email.toLowerCase() === email.toLowerCase() && u.password === password);
      if (user) {
        localStorage.setItem('ielts_current_user_email', user.email);
        localStorage.setItem('ielts_current_user_name', user.name);
        localStorage.setItem('ielts_user_membership_precise', user.plan);
        onLogin();
        toast.success("Tizimga muvaffaqiyatli kirdingiz! 🔓");
        navigate('/dashboard');
      } else {
        toast.error("Xatolik: Email yoki parol noto'g'ri!");
      }
    }
  };

  return (
    <div className="auth-page-container">
      <div className="auth-box-wrapper">
        
        {/* Left Panel: Welcome & Study Vector */}
        <div className="auth-left-panel">
          <div className="auth-left-content">
            <div className="auth-illustration-box">
              <div className="desk-vector">
                <div className="computer-screen">
                  <div className="screen-inner">👨‍💻</div>
                </div>
                <div className="desk-line"></div>
              </div>
            </div>
            <h3>Exam Mastery Hub</h3>
            <p>Unleash Your Academic Success with IELTS Hub's Exam Excellence Platform</p>
            
            {/* Dots matches mock */}
            <div className="auth-slider-dots">
              <span className="slider-dot active"></span>
              <span className="slider-dot"></span>
              <span className="slider-dot"></span>
            </div>
          </div>
        </div>

        {/* Right Panel: Interactive Form */}
        <div className="auth-right-panel">
          
          {/* Back to Home Page link */}
          <button className="back-to-home-link" onClick={() => navigate('/')}>
            <ArrowLeft size={16} /> Bosh sahifaga qaytish
          </button>

          <div className="auth-logo-row">
            <div className="logo-icon-circle-small">
              <Target size={18} color="#8b5cf6" />
            </div>
            <span className="auth-logo-text">IELTS HUB</span>
          </div>

          <h2 className="auth-form-title">
            {authMode === 'login' ? 'Tizimga Kirish' : 'Ro\'yxatdan O\'tish'}
          </h2>
          <p className="auth-form-subtitle">
            {authMode === 'login' ? 'Akkauntingizga kirib, mashqlarni davom ettiring' : 'Bepul hisob yarating va tayyorgarlikni boshlang'}
          </p>

          <form className="auth-form" onSubmit={handleAuthSubmit}>
            {authMode === 'register' && (
              <div className="form-group">
                <label>Foydalanuvchi ismi</label>
                <input 
                  type="text" 
                  placeholder="Ismingizni kiriting" 
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  required
                />
              </div>
            )}

            <div className="form-group">
              <label>Email</label>
              <input 
                type="email" 
                placeholder="john@example.com" 
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                required
              />
            </div>

            <div className="form-group">
              <div className="label-row">
                <label>Parol</label>
                {authMode === 'login' && (
                  <a href="#forgot" className="forgot-pass-link" onClick={(e) => { e.preventDefault(); toast.success("Parolni tiklash havolasi emailingizga yuborildi!"); }}>
                    Forgot password?
                  </a>
                )}
              </div>
              <div className="password-input-wrapper">
                <input 
                  type={showPassword ? 'text' : 'password'} 
                  placeholder="********" 
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  required
                />
                <button 
                  type="button" 
                  className="pwd-toggle-btn"
                  onClick={() => setShowPassword(!showPassword)}
                >
                  {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
                </button>
              </div>
            </div>

            <button type="submit" className="auth-submit-btn">
              {authMode === 'login' ? 'Sign in' : 'Sign up'}
            </button>
          </form>

          <div className="auth-divider">
            <span>or</span>
          </div>

          {/* Google Sign In matching mock */}
          <button type="button" className="auth-google-btn" onClick={() => handleGoogleLogin()}>
            <svg className="google-icon-svg" viewBox="0 0 24 24" width="18" height="18">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z" />
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z" />
            </svg>
            Sign in with Google
          </button>

          <div className="auth-footer-toggle">
            {authMode === 'login' ? (
              <>
                Are you new? <span className="toggle-mode-link" onClick={() => setAuthMode('register')}>Create an Account</span>
              </>
            ) : (
              <>
                Already have an account? <span className="toggle-mode-link" onClick={() => setAuthMode('login')}>Sign In</span>
              </>
            )}
          </div>
        </div>

      </div>
    </div>
  );
};

export default Auth;
